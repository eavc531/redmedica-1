...
<div class="row">
  <div class="col-12">
    @if($type == 'no_vistas')
    <a href="{{route('calification_medic',$medico->id)}}" class="btn btn-primary disabled"><i class="far fa-comment"></i></a>
    <a href="{{route('calification_medic_viewed',$medico->id)}}" class="btn btn-success"><i class="far fa-comments"></i></a>
    @else
    <a href="{{route('calification_medic',$medico->id)}}" class="btn btn-primary"><i class="far fa-comment"></i></a>
    <a href="{{route('calification_medic_viewed',$medico->id)}}" class="btn btn-success disabled"><i class="far fa-comments"></i></a>
    @endif
     @if($type == 'no_vistas')
    <a href="{{route('mark_all_see',$medico->id)}}" class="btn btn-primary"><i class="fas fa-check"></i>
    </a>
    @endif
    <a href="{{route('show_all_comentary',$medico->id)}}" class="btn btn-success"><i class="fas fa-eye"></i>
    </a>
    <a href="{{route('hide_all_comentary',$medico->id)}}" class="btn btn-danger"><i class="fas fa-eye-slash"></i>
    </a>
  </div>
</div>
@endif
...
<div class="card mt-2">
  <div class="card-header">
    {{$value->patient['name']}} {{$value->patient['lastName']}}&nbsp;<i class="far fa-comment"></i>
  </div>
  <div class="card-body">
    {{-- @if($value->show == 'Si') --}}
    <div class="" id="comentary">
      @if(isset($value->comentary))
      <span style="color:#666">Comentario:</span></br>
      <span>{{$value->comentary}}</span>
      @else
  ...
  <div class="card-footer">
    ...
    @if($value->show == 'Si')
    <button onclick="this.disabled=true;show_comentary(this)" class="btn btn-success space-btw" style="border:solid 3px black" id="uno" name="{{$value->id}}" disabled>Mostrar Comentario a Visitantes y Marcar como vista</button>
    <button onclick="this.disabled=true;hide_comentary(this)" class="btn btn-danger space-btw" id="dos" name="{{$value->id}}">Ocultar Comentario a Visitantes y Marcar como vista</button>

    @else
    <button onclick="this.disabled=true;show_comentary(this)" class="btn btn-success space-btw" id="uno" name="{{$value->id}}">Mostrar Comentario a Visitantes y Marcar como vista</button>
    ...
    @endif
    @if ($value->viewed == 'no')
    <button onclick="this.disabled=true;checked(this)" name="{{$value->id}}" class="btn btn-warning space-btw" id="tres">Marcar como vista</button>
    @endif
    </div>
  </div>
  @endforeach
